...
case class RSPChainTestFFT_v3_Parameters (
...
trait RSPChainTestFFT_v3_Standalone extends RSPChainTestFFT_v3 {
...
class RSPChainTestFFT_v3(val params: RSPChainTestFFT_v3_Parameters) extends LazyModule()(Parameters.empty) {
  ...
  lazy val blocks = Seq(xAWRpreProc, fft, splitter, dspQueue)
  ...
  dspQueue.streamNode := asyncQueue.streamNode
...
object RSPChainTestFFT_v3_App extends App
{
  val params = RSPChainTestFFT_v3_Parameters (
    asyncQueueParams = AsyncQueueCustomParams(),
    dspQueueParams = DspQueueCustomParams(),
    preProcParams  = AXI4XwrDataPreProcParams(),
  ...
  val standaloneModule = LazyModule(new RSPChainTestFFT_v3(params) with RSPChainTestFFT_v3_Standalone)

  chisel3.Driver.execute(Array("--target-dir", "./verilog/RSPChainTestFFT_v3", "--top-name", "RSPChainTestFFT_v3"), ()=> standaloneModule.module)
}

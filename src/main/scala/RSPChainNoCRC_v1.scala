...
case class RSPChainNoCRC_v1_Parameters (
...
trait RSPChainNoCRC_v1_Standalone extends RSPChainNoCRC_v1 {
...
class RSPChainNoCRC_v1(val params: RSPChainNoCRC_v1_Parameters, val addDebugNodes: Boolean = false) extends LazyModule()(Parameters.empty) {
  ...
  // define mem
  lazy val blocks = Seq(xAWRpreProc, fft, dspQueue, windowing)
  ...
  fft.streamNode := AXI4StreamBuffer() := windowing.streamNode := AXI4StreamBuffer() := xAWRpreProc.streamNode := AXI4StreamBuffer() := dspQueue.streamNode := AXI4StreamBuffer() := asyncQueue.streamNode
...
object RSPChainNoCRC_v1_App extends App
{
  val params = RSPChainNoCRC_v1_Parameters (
    asyncQueueParams = AsyncQueueCustomParams(),
    dspQueueParams = DspQueueCustomParams(),
    preProcParams  = AXI4XwrDataPreProcParams(),
  ...
  val standaloneModule = LazyModule(new RSPChainNoCRC_v1(params, addDebugNodes= true) with RSPChainNoCRC_v1_Standalone)

  chisel3.Driver.execute(Array("--target-dir", "./verilog/RSPChainNoCRC_v1", "--top-name", "RSPChainNoCRC_v1"), ()=> standaloneModule.module)
}

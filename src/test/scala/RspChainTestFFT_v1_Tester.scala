...
class RSPChainTestFFT_v1_Tester(
  dut: RSPChainTestFFT_v1 with RSPChainTestFFT_v1_Standalone,
  params: RSPChainTestFFT_v1_Parameters,
  inFileName: String,
  beatBytes: Int = 4,
  ...
  memWriteWord(params.dspQueueAddress.base, 3072)
...
class RSPChainTestFFT_v1_Spec extends AnyFlatSpec with Matchers {

  val params = RSPChainTestFFT_v1_Parameters (
    preProcParams  = AXI4XwrDataPreProcParams(),
    dspQueueParams = DspQueueCustomParams(queueDepth = 5120, progFull = true),
    ...
    val testModule = LazyModule(new RSPChainTestFFT_v1(params) with RSPChainTestFFT_v1_Standalone)
    ...
    it should "Test simple rsp chain with combiner and splitter" in {
      chisel3.iotesters.Driver.execute(Array("-tiwv", "-tbn", "verilator", "-tivsuv"), () => testModule.module) {
        c => new RSPChainTestFFT_v1_Tester(dut = testModule,
                                            beatBytes = 4,
                                            params = params,
